...
<script id="templates/K3.html" type="text/ng-template">
...
-->
	      ...
	      <%-- header --%>
          <div class="playTitle" id="title_k3"><%--ng-click="playList.show()"--%>
              <span id="twowords" style="vertical-align:text-bottom">玩法</span>
              <span>
                  {{methodnameDisplay}}
          ...
          <div class="nowPage">
			...
			<div class="rightTopArea">
             <div class="rightTopArr"></div>
                 	<ins></ins>
         		<a ng-click="jumpCheck('{{item.playId}}','{{item.id}}')" ng-repeat="item in sameTypeGames">{{item.gameNickName}}</a>
              </div>

      <ion-nav-view name="menuContent">
<div class="wrap_playList_k3">
    <div class="lottery-wf boxA">
      <div class="wrapPlayType clearfix">
          <a href="javascript:;" class="playA {{ selNav.title == playNav.title ? 'active' : ''}}"
             ng-click="changeSelNav(playNav);"
             ng-repeat="playNav in betPlays" onclick="playListHide();">
             <div class="type_title">{{playNav.title}}</div>
             <div class="bet_div"><span class="bet_span">赔率</span>{{playNav.showRate}}倍</div>
             <div class="clearfix wrap_type_dice">
                 <div class="wrap_bet_dice">
                     <img src="/theme/wap/img/yingjia/diceK3.png">
                 </div>
                 <div class="wrap_bet_dice">
                     <img src="/theme/wap/img/yingjia/diceK3.png">
                 </div>
        ...
        <div class="wrap_playtype"style="display:none">
            <div ng-repeat="playNav2 in selNav.label" class="clearfix dash_border">
              <div class="lottery-wf-t nb">{{playNav2.gtitle}}</div>
              <div class="lottery-wf ball_fr">
                <a href="javascript:;" ng-repeat="item in playNav2.label"
                   class="{{ item.methodid == selPlay.methodid ? 'active' : ''}}"ng-click="changeSelPlay(item); playList.hide();">{{item.desc}}</a>
              </div>
            </div>
        </div>
...
<!-- 賽車表演區結束 -->
<div class="wrapNoClock clearfix">
    <div class="openNo">
        <a href="javascript:;" class="item item-icon-right lottery-choose long-choose" ng-click="numbersList2.show()">
 	      <div class="middleArea">
 		 	  <div ng-show="isIssueNeedFlag" class="font7em">
                  <font class="assertive fff"> {{ lastIssueDisplay}} </font>
                  期开奖号码
              </div>
          	  <span ng-show="!isIssueNeedFlag">开奖号码</span>
          	  <div class="ballArea clearfix">
			      <!-- <font ng-repeat="small in lastResultShow" class="{{small.class}} fff">{{small.no}}</font> -->
                  <!-- <div class="wrapDice" ng-repeat="small in lastResultShow">
                      <img src="/theme/wap/img/yingjia/diceK3.png" class="wrapDiceImg{{small.no}}">
                  </div> -->
                  <div class="wrapDice diceImg0">
                      <img src="/theme/wap/img/yingjia/diceK3.png" >
                  </div>
                  <div class="wrapDice diceImg1">
                      <img src="/theme/wap/img/yingjia/diceK3.png" >
                  </div>
                  <div class="wrapDice diceImg2">
                      <img src="/theme/wap/img/yingjia/diceK3.png" >
                  </div>
                  <i class="icon ion-arrow-down-b k3DownArr"></i>
		      </div>
          </div>
		  <%-- <i class="icon ion-chevron-down icon-accessory"></i> --%>
        </a>
    </div>
    <div class="reciprocalClock font7em">
      <a href="javascript:;" class="item" ng-show="isIssueNeedFlag">
          <font class="assertive fff">{{currentTempIssue}}</font>
		     期投注截止
          <div id="clock">00:00:00</div>
      </a>
    </div>
</div>
<div class="wrap_open_k3no">
    <div class="k3Title clearfix">
        <div>期号</div>
        <div>开奖号码</div>
        <div>和值</div>
        <div>大小</div>
    ...
    <div class="wrap_k3no" ng-repeat="result in lotteryResults.top10History">
        <div class="k3_everyOpen">{{result.issue}}</div>
        <div class="wrap_diceK3 clearfix">
            <div class="wrapDicek3" ng-repeat="num in result.resultDisplay track by $index">
                <!-- <img src="/theme/wap/img/yingjia/diceK3.png" class="open_dice{{num}}"> -->
                <div class="diceAlone{{num}}"></div>
            </div>
        </div>
        <div>{{result.k3TotalResult}}</div>
        <div>{{result.k3BigSmallWord}}</div>
        <div>{{result.k3OddEvenWord}}</div>
    </div>
</div>
      ...
      <div class="forouter">
      <div class="lottery-tip">{{ selPlay.methoddesc}}</div>
      <!-- {{selPlay.prizeDesc }} -->
      <!-- <div class="betrecord_btn pull-right">
        <a href="../../pro/betCenter/bet?pageGameId=${game.id}">
        <img src="/theme/wap/img/betrecord_K3.png" alt="">
          <p>投注记录</p>
        </a>
      </div>  -->
...
' || gameId == 'JX11X5' || gameId == 'GD11X5' || gameId == 'SD11X5' || gameId == 'SH11X5' || gameId == 'HLJ11X5' || gameId == 'XJ11X5' || gameId == 'JS11X5')?'每注的号码之间使用空格分隔，注与注之间使用逗号或者分号或者回车换行分隔，注意：不足2位要在前面加0。':'注与注之间使用逗号或者分号或者回车换行分隔'
      ...
      <div class="ball-list" ng-style="{width: selPlay.methodname =='K3TH3TS' || selPlay.methodname =='K3LH3TS'? '60%':'100%' , 'margin-left': selPlay.methodname =='K3TH3TS' || selPlay.methodname =='K3LH3TS' ? '18%':'' }  " >
        <div ng-repeat="layout in selPlay.selectarea.layout">
          ...
          <div class="{{optionData('class')}}{{titleShow ? ' classTitle' : ''}}">
			<div class="row" ng-repeat="row in layout.noArray" >
              <div class="bet_green col col-{{optionData('col')}} default {{layout.selNumbers[n] ? ' active' : ''}}" ng-repeat="n in row" ng-click="selectNumber(layout, n)">
                <a href="javascript:;" class="{{getBallColor(n)}}{{layout.selNumbers[n] ? ' active' : ''}}{{optionData('contentClass')}}">{{n}}</a>
                <span ng-if=optionData('showPlus') class="{{getBallColor(n)}}{{layout.selNumbers[n] ? ' active' : ''}}">赔{{getRebuildShow(n)}}</span>
              </div>
            </div>
          </div>
          <%-- <div class="line"></div> --%>
      ...
      <div class="blank_gre" style="width:100%;height:90px;background-color:rgb(49, 116, 85);"></div>
    </ion-content>
    <div class="inputMoney" ng-if="displaySelStr!=''">
        <div class="nowChose">
            <span>当前选号</span>
            <span class="nowText"> {{displaySelStr}} </span>
        </div>
        <div class="everyBet clearfix">
            <div>每注金额</div>
            <div><input type="tel" ng-change="refreshBetCount()" ng-model="betContent.times" name="" value="" maxlength="7"></div>
            <div class="sm-text" ng-if="!(topReward > 0)">请输入要投注的金额</div>
			<div class="sm-text" ng-if="topReward > 0">最高可中<span class='nowText'>{{topReward}}</span>元</div>
        </div>
    </div>
	<nav class="tabs tabs-icon-top tabs-dark tac" style="">
            <div class="bottom-bar flFoot clearfix">
                  ...
                  <div class="clean" ng-click="clearBetContent()">清空</div>
                  <div class="moneyLeft clearfix">
                      <span>{{ betContent.count }}注 共{{ betContent.totalMoney }}元</span>
                      <span class="blackTran"></span>
                  </div>
                  <div class="wrapBucket">
                      <button class="btn_tran transparent" ng-click="saveSelBet()">
                          马上投注
                      </button>
                  </div>
              </div>
    ...
    <footer class="bar bar-footer bar-stable has-tabs showBlock">
      ...
      <div class="bottom-button-g">
		...
		<%-- footer --%>
        <nav class="tabs tabs-icon-top tabs-dark tac">
            <div class="bottom-bar flFoot clearfix">
                  <button id="btnCR" class="button button-energized button-outline bottom-button bottom-button-l" ng-click="randomBetSelect()">机选</button>
                  <div class="clean"  ng-click="clearBetContent()">清空</div>
                  <div class="moneyLeft clearfix">
                      <div class="wrapFootBet">
                          <div class="wrapBet">
                          共{{selBetCount}}注 ,<span class="showbetMoney">{{selBetMoney}}</span>元
                          </div>
                          <div class="selectedNo">
                              123123
                          </div>
                      </div>
                      <span class="blackTran"></span>
...
<%--历史开奖记录 --%>
<script id="templates/numbersListK3.html" type="text/ng-template">
  <ion-modal-view>
...
<%-- 所有采种列表 --%>
<script id="templates/lotteryListK3.html" type="text/ng-template">
  <ion-modal-view>
    <ion-header-bar class="bar bar-header bar-light">
...
<%-- 彩种玩法列表 --%>
<script id="templates/playListK3.html" type="text/ng-template">
  <ion-modal-view class="wrapPlayNav">
    ...
    <ion-content>
          <div class="lottery-wf boxA">
            <div class="wrapPlayType clearfix">
                <a href="javascript:;" class="playA {{ selNav.title == playNav.title ? 'active' : ''}}"
                   ng-click="changeSelNav(playNav);playList.hide();"
                   ng-repeat="playNav in betPlays">
                   <div class="type_title">{{playNav.title}}</div>
                   <div class="bet_div"><span class="bet_span">赔率</span>{{playNav.showRate}}倍</div>
                   <div class="clearfix wrap_type_dice">
                       <div class="wrap_bet_dice">
...
<%-- <nav class="tabs tabs-icon-top tabs-dark tac hobet" style="">
            <div class="bottom-bar flFoot clearfix">
                  <button id="btnCR" class="button button-energized button-outline bottom-button bottom-button-l" ng-click="randomBetSelect()">机选</button>
                  <div class="clean" ng-click="clearBetContent()">清空</div>
...
<%-- 注单 --%>
<script id="templates/betListK3.html" type="text/ng-template">
<ion-modal-view>
<ion-header-bar class="bar bar-header bar-light">
